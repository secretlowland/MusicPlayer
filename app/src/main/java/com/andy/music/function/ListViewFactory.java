...
import com.andy.music.listener.MusicListListener;
...
public class ListViewFactory {
    ...
    public static View createView(int viewType) {
        ...
        int[] to = { R.id.tv_cell_double_line_first, R.id.tv_cell_double_line_second };

        switch (viewType) {
            case LIST_VIEW_MUSIC:
                listView.setOnItemClickListener(new MusicListListener());
                adapter = new MusicListAdapter(context, MusicList.getInstance(MusicList.MUSIC_LIST_LOCAL).getList(), R.layout.music_list_cell);
                break;
            case LIST_VIEW_SINGER:
                adapter = new SimpleCursorAdapter(context, R.layout.cell_double_line, cursor, from, to, 0);
                break;
